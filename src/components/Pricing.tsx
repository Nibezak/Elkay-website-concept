    ...
    <section className={"w-full flex items-center justify-center bg-white py-[120px] px-4 md:px-[135px] overflow-hidden " + (className || "")}>
      <motion.div className="w-full max-w-[1440px] flex flex-col items-center gap-[80px]" initial="initial" whileInView="animate" viewport={{ once: true, margin: "-100px" }} variants={containerVariants}>
        <div className="flex flex-col items-center gap-4 max-w-[720px]">
          <motion.h2 variants={itemVariants} className="text-[#012E1B] text-[48px] font-bold leading-[120%] text-center tracking-[-0.3px] font-serif">
            Partner for <br /> Industrial Growth
          </motion.h2>
          <motion.p variants={itemVariants} className="text-[#012E1B] text-[18px] leading-[28px] opacity-80 text-center font-normal font-sans">
            ...
            return (
              <motion.div key={plan.id} variants={itemVariants} onMouseEnter={() => setActiveCardId(plan.id)} whileHover={{ y: -6, boxShadow: "0px 20px 40px rgba(0,0,0,0.08)" }} transition={{ duration: 0.4 }} className={"flex flex-col justify-between p-8 rounded-[24px] transition-all duration-500 ease-in-out border " + (isActive ? "bg-[#012E1B] text-white border-transparent" : "bg-white text-[#012E1B] border-[#E5E7EB]")}>
                    ...
                    <ul className="flex flex-col gap-4">
                      {plan.features.map((feature, idx) => (
                        <li key={idx} className="flex items-center gap-3">
                          {feature.included ? <CheckCircle2 size={20} className={"transition-colors duration-500 " + (isActive ? "text-[#85FA6D]" : "text-[#012E1B]")} /> : <XCircle size={20} className="opacity-40" />}
                ...
                <div className="flex flex-col gap-6 mt-8">
                  ...
                  <motion.a href="#" whileHover={{ scale: 1.02, boxShadow: isActive ? "0px 12px 24px rgba(133, 250, 109, 0.4)" : "0px 12px 24px rgba(0,0,0,0.12)" }} transition={{ duration: 0.3 }} className={"group flex items-center justify-center gap-2 py-[14px] px-6 rounded-full text-[16px] font-medium transition-all duration-500 ease-in-out cursor-pointer " + (isActive ? "bg-[#85FA6D] text-[#012E1B]" : "bg-[#EDFBEA] text-[#012E1B]")}>
                    {plan.buttonText}
